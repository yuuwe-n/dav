...
std::unique_ptr<VerilatedTraceConfig> VminiALU_top_tb::traceConfig() const {
...
void VminiALU_top_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void VminiALU_top_tb___024root__trace_init_top(VminiALU_top_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    VminiALU_top_tb___024root__trace_decl_types(tracep);
    VminiALU_top_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void VminiALU_top_tb___024root__trace_register(VminiALU_top_tb___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void VminiALU_top_tb::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'VminiALU_top_tb::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    VminiALU_top_tb___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
